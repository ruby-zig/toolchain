...
extconf="$source_root/ext/io/wait/extconf.rb"
[[ -f "$extconf" && -f "$source_root/ext/io/wait/wait.c" ]] || {
  printf 'run this adapter from the io-wait repository root\n' >&2
  exit 66
}
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'io-wait adapter has not been certified for Zig target %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
...
for command_name in ruby make file readelf sha256sum; do
  command -v "$command_name" >/dev/null 2>&1 || {
    printf 'io-wait adapter requires %s\n' "$command_name" >&2
    exit 69
  }
done
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'io-wait adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/io-wait-$RZ_ZIG_TARGET"
...
artifact="$build_dir/wait.so"
[[ -s "$artifact" ]] || {
  printf 'io-wait extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
cp "$artifact" "$artifact_dir/wait.so"
artifact="$artifact_dir/wait.so"

format="$(file -b "$artifact")"
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected io-wait artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_wait ]]; then
  printf 'io-wait artifact must export only Init_wait; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'io-wait artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'io-wait requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi
...
printf 'io-wait GNU compatibility artifact loaded; runtime tests passed; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
