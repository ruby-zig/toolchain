...
[[ "$RZ_SOURCE_REF_NAME" == master ]] || {
  printf 'io-nonblock adapter only supports the tracked master branch\n' >&2
  exit 78
}
git -C "$source_root" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
  printf 'io-nonblock source must be a Git worktree\n' >&2
  exit 66
}

actual_sha="$(git -C "$source_root" rev-parse HEAD)"
[[ "$actual_sha" == "$RZ_SOURCE_REF" ]] || {
  printf 'io-nonblock source is %s, expected %s\n' "$actual_sha" "$RZ_SOURCE_REF" >&2
  exit 78
}

status="$(git -C "$source_root" status --porcelain=v1 --untracked-files=all)"
[[ -z "$status" ]] || {
  printf 'io-nonblock adapter requires a clean source worktree:\n%s\n' "$status" >&2
  exit 73
}

printf 'io-nonblock source accepted: master@%s\n' "$actual_sha"
