...
[[ "$RZ_SOURCE_REF_NAME" == master ]] || {
  printf 'fiddle adapter only supports the tracked master branch\n' >&2
  exit 78
}
git -C "$source_root" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
  printf 'fiddle source must be a Git worktree\n' >&2
  exit 66
}

actual_sha="$(git -C "$source_root" rev-parse HEAD)"
[[ "$actual_sha" == "$RZ_SOURCE_REF" ]] || {
  printf 'fiddle source is %s, expected %s\n' \
...
[[ -z "$status" ]] || {
  printf 'fiddle adapter requires a clean source worktree:\n%s\n' "$status" >&2
  exit 73
}

printf 'fiddle source accepted: master@%s\n' "$actual_sha"
