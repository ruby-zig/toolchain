...
extconf="$source_root/ext/fcntl/extconf.rb"
[[ -f "$extconf" && -f "$source_root/ext/fcntl/fcntl.c" ]] || {
  printf 'run this adapter from the fcntl repository root\n' >&2
  exit 66
}
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'fcntl adapter has not been certified for Zig target %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
...
for command_name in ruby make file readelf sha256sum; do
  command -v "$command_name" >/dev/null 2>&1 || {
    printf 'fcntl adapter requires %s\n' "$command_name" >&2
    exit 69
  }
done
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'fcntl adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/fcntl-$RZ_ZIG_TARGET"
...
artifact="$build_dir/fcntl.so"
[[ -s "$artifact" ]] || {
  printf 'fcntl extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
cp "$artifact" "$artifact_dir/fcntl.so"
artifact="$artifact_dir/fcntl.so"

format="$(file -b "$artifact")"
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected fcntl artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_fcntl ]]; then
  printf 'fcntl artifact must export only Init_fcntl; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'fcntl artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'fcntl requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi
...
printf 'fcntl GNU artifact loaded and passed runtime tests; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
