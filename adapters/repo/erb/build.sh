...
extconf="$source_root/ext/erb/escape/extconf.rb"
source_file="$source_root/ext/erb/escape/escape.c"
[[ -f "$extconf" && -f "$source_file" ]] || {
  printf 'run this adapter from the ERB repository root\n' >&2
  exit 66
}
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'ERB adapter has not been certified for Zig target %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'ERB adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/erb-$RZ_ZIG_TARGET"
artifact_dir="$artifact_root/$RZ_ZIG_TARGET/erb"
...
artifact="$build_dir/escape.so"
[[ -s "$artifact" ]] || {
  printf 'ERB escape extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
cp "$artifact" "$artifact_dir/escape.so"
artifact="$artifact_dir/escape.so"

format="$(file -b "$artifact")"
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected ERB escape artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_escape ]]; then
  printf 'ERB escape artifact must export only Init_escape; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'ERB escape artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'ERB escape requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi
...
ruby --disable-gems -e '
  ...
  plain = +"ruby-zig"
  abort "native ERB no-op escaping changed object identity" unless
    ERB::Escape.html_escape(plain).equal?(plain)
  convertible = Object.new
  def convertible.to_s
    "<zig>"
  end
  converted = ERB::Escape.html_escape(convertible)
  abort "native ERB conversion escaping failed" unless converted == "&lt;zig&gt;"

  encoded = "<&>".encode(Encoding::ISO_8859_1)
  result = ERB::Escape.html_escape(encoded)
  abort "native ERB escaping lost encoding" unless result.encoding == encoded.encoding
' "$artifact"

[[ "$(git -C "$source_root" rev-parse HEAD)" == "$RZ_SOURCE_REF" ]] || {
  printf 'ERB source commit changed during the build\n' >&2
  exit 73
}
git -C "$source_root" diff --quiet --ignore-submodules --
git -C "$source_root" diff --cached --quiet --ignore-submodules --

printf 'ERB GNU artifact loaded and passed runtime tests; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
