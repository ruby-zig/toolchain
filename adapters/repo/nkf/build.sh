...
extconf="$source_root/ext/nkf/extconf.rb"
source_inputs=(
  "$source_root/ext/nkf/nkf.c"
  "$source_root/ext/nkf/nkf-utf8/config.h"
  "$source_root/ext/nkf/nkf-utf8/nkf.c"
  "$source_root/ext/nkf/nkf-utf8/nkf.h"
  "$source_root/ext/nkf/nkf-utf8/utf8tbl.c"
  "$source_root/ext/nkf/nkf-utf8/utf8tbl.h"
)
[[ -f "$extconf" ]] || {
  printf 'run this adapter from the nkf repository root\n' >&2
  exit 66
}
for source_input in "${source_inputs[@]}"; do
  [[ -f "$source_input" ]] || {
    printf 'nkf pinned source input is missing: %s\n' "$source_input" >&2
    exit 66
  }
done
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'nkf adapter is implemented only for Zig target x86_64-linux-gnu.2.17; got %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
[[ -x "$RZ_ZIG" ]] || {
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'nkf adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/nkf-$RZ_ZIG_TARGET"
...
artifact="$build_dir/nkf.so"
[[ -s "$artifact" ]] || {
  printf 'nkf extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
cp "$artifact" "$artifact_dir/nkf.so"
artifact="$artifact_dir/nkf.so"

format="$(file -b "$artifact")"
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected nkf artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_nkf ]]; then
  printf 'nkf artifact must export only Init_nkf; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'nkf artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'nkf requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi
...
[[ -s "$receipt_file" ]] || {
  printf 'nkf build produced no Zig wrapper receipts: %s\n' "$receipt_file" >&2
  exit 70
}
grep -Eq $'\ttool=cc\t.*\toperation=compile\t' "$receipt_file" || {
  printf 'nkf build has no Zig C compilation receipt\n' >&2
  exit 70
}
grep -Eq $'\ttool=cc\t.*\toperation=compile\t.*\targv=.*ext/nkf/nkf\\.c([[:space:]]|$)' \
  "$receipt_file" || {
  printf 'nkf build has no exact ext/nkf/nkf.c compilation receipt\n' >&2
  exit 70
}
compile_count="$(
  awk '$0 ~ /\ttool=cc\t/ && $0 ~ /\toperation=compile\t/ {count++} END {print count + 0}' \
    "$receipt_file"
)"
[[ "$compile_count" == 1 ]] || {
  printf 'nkf build must compile one wrapper translation unit; got %s\n' \
    "$compile_count" >&2
  exit 70
}
if grep -Eq $'\ttool=cc\t.*\toperation=compile\t.*\targv=.*nkf-utf8/(nkf|utf8tbl)\\.c' \
  "$receipt_file"; then
  printf 'nkf vendored C engine must be included by nkf.c, not compiled separately\n' >&2
  exit 70
fi
grep -Eq $'\ttool=shared\t.*\toperation=link\t' "$receipt_file" || {
  printf 'nkf build has no Zig shared-link receipt\n' >&2
  exit 70
}
...
[[ "$actual_sha" == "$RZ_SOURCE_REF" ]] || {
  printf 'nkf source HEAD changed during the build\n' >&2
  exit 70
}
git -C "$source_root" diff --quiet --ignore-submodules -- || {
  printf 'nkf build modified tracked source files\n' >&2
  exit 70
}
git -C "$source_root" diff --cached --quiet --ignore-submodules -- || {
  printf 'nkf build modified the source index\n' >&2
  exit 70
}

printf 'nkf GNU artifact loaded and passed runtime tests; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
