...
extconf="$source_root/ext/pathname/extconf.rb"
source_file="$source_root/ext/pathname/pathname.c"
[[ -f "$extconf" && -f "$source_file" ]] || {
  printf 'run this adapter from the pathname repository root\n' >&2
  exit 66
}
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'pathname adapter has not been certified for Zig target %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
...
for command_name in ruby make file readelf sha256sum; do
  command -v "$command_name" >/dev/null 2>&1 || {
    printf 'pathname adapter requires %s\n' "$command_name" >&2
    exit 69
  }
done
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'pathname adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/pathname-$RZ_ZIG_TARGET"
...
artifact="$build_dir/pathname.so"
[[ -s "$artifact" ]] || {
  printf 'pathname extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
cp "$artifact" "$artifact_dir/pathname.so"
artifact="$artifact_dir/pathname.so"

format="$(file -b "$artifact")"
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected pathname artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_pathname ]]; then
  printf 'pathname artifact must export only Init_pathname; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'pathname artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'pathname requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi

# Load only the staged native extension. A small Ruby initializer supplies the
# object state needed to exercise the C methods without loading an installed
# pathname extension.
...
ruby --disable-gems -e '
  ...
  abort "exact pathname artifact was not loaded" unless loaded.include?(artifact)
  ...
  left = Pathname.new("/usr/bin/perl")
  right = Pathname.new("/usr/bin/ruby")
  abort "native pathname comparison failed" unless (left <=> right) == -1
  replaced = left.sub("perl", "ruby")
  abort "native pathname substitution failed" unless
    replaced.instance_variable_get(:@path) == "/usr/bin/ruby"
  block_replaced = left.sub(/perl/) { "zig" }
  abort "native pathname block substitution failed" unless
    block_replaced.instance_variable_get(:@path) == "/usr/bin/zig"
' "$artifact"

[[ "$(git -C "$source_root" rev-parse HEAD)" == "$RZ_SOURCE_REF" ]] || {
  printf 'pathname source commit changed during the build\n' >&2
  exit 73
}
git -C "$source_root" diff --quiet --ignore-submodules --
git -C "$source_root" diff --cached --quiet --ignore-submodules --

printf 'pathname GNU artifact loaded and passed runtime tests; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
