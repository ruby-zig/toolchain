...
command -v git >/dev/null 2>&1 || {
  printf 'debug source contract test requires git\n' >&2
  exit 69
}
...
printf 'nested\n' >"$source_root/nested/source.txt"
git -C "$source_root" add -- source.txt nested/source.txt
...
if RZ_SOURCE_REF="${sha^^}" RZ_SOURCE_REF_NAME=master \
  bash "$root/source-contract.sh" "$source_root" >/dev/null 2>&1; then
  printf 'source contract accepted an uppercase SHA\n' >&2
  exit 1
fi
if RZ_SOURCE_REF="$sha" RZ_SOURCE_REF_NAME=master \
  bash "$root/source-contract.sh" "$source_root/nested" >/dev/null 2>&1; then
  printf 'source contract accepted a worktree subdirectory\n' >&2
  exit 1
fi
...
printf 'debug source contract tests passed\n'
