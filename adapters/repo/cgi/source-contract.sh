...
[[ "$RZ_SOURCE_REF_NAME" == master ]] || {
  printf 'cgi adapter only supports the tracked master branch\n' >&2
  exit 78
}
git -C "$source_root" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
  printf 'cgi source must be a Git worktree\n' >&2
  exit 66
}

actual_sha="$(git -C "$source_root" rev-parse HEAD)"
[[ "$actual_sha" == "$RZ_SOURCE_REF" ]] || {
  printf 'cgi source is %s, expected %s\n' "$actual_sha" "$RZ_SOURCE_REF" >&2
  exit 78
}

status="$(git -C "$source_root" status --porcelain=v1 --untracked-files=all)"
[[ -z "$status" ]] || {
  printf 'cgi adapter requires a clean source worktree:\n%s\n' "$status" >&2
  exit 73
}

printf 'cgi source accepted: master@%s\n' "$actual_sha"
