...
extconf="$source_root/ext/cgi/escape/extconf.rb"
[[ -f "$extconf" && -f "$source_root/ext/cgi/escape/escape.c" ]] || {
  printf 'run this adapter from the cgi repository root\n' >&2
  exit 66
}
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'cgi adapter is certified only for Zig target x86_64-linux-gnu.2.17; got %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
[[ -x "$RZ_ZIG" ]] || {
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'cgi adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/cgi-$RZ_ZIG_TARGET"
artifact_dir="$artifact_root/$RZ_ZIG_TARGET/cgi"
...
[[ -s "$artifact" ]] || {
  printf 'cgi extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
...
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected cgi artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_escape ]]; then
  printf 'cgi artifact must export only Init_escape; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'cgi artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'cgi requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi
...
[[ -s "$receipt_file" ]] || {
  printf 'cgi build produced no Zig wrapper receipts: %s\n' "$receipt_file" >&2
  exit 70
}
grep -Eq $'\ttool=cc\t.*\toperation=compile\t' "$receipt_file" || {
  printf 'cgi build has no Zig C compilation receipt\n' >&2
  exit 70
}
grep -Eq $'\ttool=shared\t.*\toperation=link\t' "$receipt_file" || {
  printf 'cgi build has no Zig shared-link receipt\n' >&2
  exit 70
}
...
[[ "$actual_sha" == "$RZ_SOURCE_REF" ]] || {
  printf 'cgi source HEAD changed during the build\n' >&2
  exit 70
}
git -C "$source_root" diff --quiet --ignore-submodules -- || {
  printf 'cgi build modified tracked source files\n' >&2
  exit 70
}
git -C "$source_root" diff --cached --quiet --ignore-submodules -- || {
  printf 'cgi build modified the source index\n' >&2
  exit 70
}

printf 'cgi GNU artifact loaded and passed runtime tests; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
