...
extconf="$source_root/ext/iconv/extconf.rb"
source_file="$source_root/ext/iconv/iconv.c"
[[ -f "$extconf" && -f "$source_file" ]] || {
  printf 'run this adapter from the iconv repository root\n' >&2
  exit 66
}
...
[[ "$RZ_ZIG_TARGET" == x86_64-linux-gnu.2.17 ]] || {
  printf 'iconv adapter has not been certified for Zig target %s\n' \
    "$RZ_ZIG_TARGET" >&2
  exit 78
}
[[ -d "$RZ_RECEIPT_DIR" ]] || {
...
for command_name in ruby make file readelf sha256sum; do
  command -v "$command_name" >/dev/null 2>&1 || {
    printf 'iconv adapter requires %s\n' "$command_name" >&2
    exit 69
  }
done
...
[[ "$actual_ruby_version" == "$expected_ruby_version" ]] || {
  printf 'iconv adapter requires Ruby %s; got %s\n' \
...
build_dir="$build_root/iconv-$RZ_ZIG_TARGET"
...
artifact="$build_dir/iconv.so"
[[ -s "$artifact" ]] || {
  printf 'iconv extension was not produced: %s\n' "$artifact" >&2
  exit 70
}
cp "$artifact" "$artifact_dir/iconv.so"
artifact="$artifact_dir/iconv.so"

format="$(file -b "$artifact")"
[[ "$format" == *'ELF 64-bit LSB shared object, x86-64'* ]] || {
  printf 'unexpected iconv artifact format: %s\n' "$format" >&2
  exit 70
}
...
if [[ ${#init_exports[@]} -ne 1 || "${init_exports[0]:-}" != Init_iconv ]]; then
  printf 'iconv artifact must export only Init_iconv; got: %s\n' \
    "${init_exports[*]:-none}" >&2
  exit 70
fi
...
if grep -Eq '\((RPATH|RUNPATH)\)' <<<"$dynamic"; then
  printf 'iconv artifact must not contain RPATH or RUNPATH\n' >&2
  exit 70
fi

if readelf -d "$artifact" | grep -Fq 'libiconv'; then
  printf 'GNU iconv artifact unexpectedly depends on external libiconv\n' >&2
  exit 70
fi
...
if [[ -n "$glibc_max" ]]; then
  ...
  [[ "$newest" == 2.17 ]] || {
    printf 'iconv requires GLIBC_%s, above the declared 2.17 ceiling\n' \
      "$glibc_max" >&2
    exit 70
  }
fi
...
[[ "$(git -C "$source_root" rev-parse HEAD)" == "$RZ_SOURCE_REF" ]] || {
  printf 'iconv source SHA changed during the build\n' >&2
  exit 78
}
...
printf 'iconv GNU artifact passed conversion runtime tests; glibc_max=%s\n' \
  "${glibc_max:-none}"
sha256sum "$artifact"
