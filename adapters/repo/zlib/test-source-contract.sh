...
command -v git >/dev/null 2>&1 || {
  printf 'zlib source contract test requires git\n' >&2
  exit 69
}
...
if RZ_SOURCE_REF="$sha" RZ_SOURCE_REF_NAME=master \
  ...
  printf 'source contract accepted a nested directory as the source root\n' >&2
  exit 1
fi
...
printf 'zlib source contract tests passed\n'
